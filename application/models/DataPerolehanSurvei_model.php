...
class DataPerolehanSurvei_model extends CI_Model
{
    ...
    private function _get_datatables_query($table_identity, $profil_responden)
    {
        ...
        $this->db->select("*, id_responden, is_submit, kode_surveyor, responden_$table_identity.uuid AS uuid_responden, waktu_isi, (SELECT first_name FROM users WHERE id = surveyor.id_user) AS first_name, (SELECT last_name FROM users WHERE id = surveyor.id_user) AS last_name, 
