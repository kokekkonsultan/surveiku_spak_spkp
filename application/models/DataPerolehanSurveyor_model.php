...
class DataPerolehanSurveyor_model extends CI_Model
{
    ...
    private function _get_datatables_query($profil_responden, $table_identity, $user)
    {
        ...
        $this->db->select("*, id_responden, nama_lengkap, is_submit, kode_surveyor, responden_$table_identity.uuid AS uuid_responden, waktu_isi, (SELECT first_name FROM users WHERE id = surveyor.id_user) AS first_name, (SELECT last_name FROM users WHERE id = surveyor.id_user) AS last_name, $query_profil, survey_$table_identity.id AS id_survey");
        ...
        $this->db->join("users u", "surveyor.id_user = u.id");
