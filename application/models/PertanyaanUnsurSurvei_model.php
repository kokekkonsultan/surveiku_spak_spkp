...
class PertanyaanUnsurSurvei_model extends CI_Model
{
    ...
    private function _get_datatables_query($table_identity)
    {
        ...
        $this->db->where('is_produk', $this->session->userdata('is_produk'));
    ...
    public function dropdown_sub_unsur_pelayanan($table_identity, $is_produk)
    {
        $query = $this->db->query("SELECT *
        FROM (SELECT unsur_pelayanan_$table_identity.id AS id_unsur, nomor_unsur, nama_unsur_pelayanan, if(id_unsur_pelayanan != '', 1, 2) AS unsur_turunan, is_produk
        ...
        WHERE unsur_modif.unsur_turunan = 2 && is_produk = $is_produk");
