...
class OlahData_model extends CI_Model
{
    ...
    private function _get_datatables_query($table_identity)
    {
        ...
        $this->db->from("(
            (SELECT *,
            (SELECT SUM(skor_jawaban) FROM jawaban_pertanyaan_unsur_$table_identity WHERE jawaban_pertanyaan_unsur_$table_identity.id_responden = responden_$table_identity.id) AS total_skor,
            ...
            WHERE jawaban_pertanyaan_unsur_$table_identity.id_responden = responden_$table_identity.id && is_produk = 1) AS total_skor_spak,
            
            (SELECT SUM(skor_jawaban)
            FROM jawaban_pertanyaan_unsur_$table_identity
            JOIN pertanyaan_unsur_pelayanan_$table_identity ON jawaban_pertanyaan_unsur_$table_identity.id_pertanyaan_unsur = pertanyaan_unsur_pelayanan_$table_identity.id
            JOIN unsur_pelayanan_$table_identity ON pertanyaan_unsur_pelayanan_$table_identity.id_unsur_pelayanan = unsur_pelayanan_$table_identity.id
            WHERE jawaban_pertanyaan_unsur_$table_identity.id_responden = responden_$table_identity.id && is_produk = 2) AS total_skor_spkp
            
            FROM responden_$table_identity
            ) AS rspdn_$table_identity)");
