...
class AuthKlien_model extends CI_Model
{
	...
	private function _get_datatables_query()
	{
		...
		$this->db->join('klasifikasi_survei ks', 'u.id_klasifikasi_survei = ks.id');
	...
	public function dropdown_parent_induk()
	{
		$query = $this->db->query("SELECT * FROM users JOIN users_groups ON users.id = users_groups.user_id WHERE users_groups.group_id = 6");

		if ($query->num_rows() > 0) {

			$dd[''] = 'Please Select';
			foreach ($query->result_array() as $row) {
				$dd[$row['user_id']] =  $row['first_name'] . ' ' . $row['last_name'] . ' - ' . $row['company'];
			}
