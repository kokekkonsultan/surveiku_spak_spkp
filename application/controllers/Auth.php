...
class Auth extends CI_Controller
{
	...
	public function ajax_list_klien()
	{
		...
		foreach ($list as $value) {
			...
			$row[] = anchor("pengguna-klien/edit-klien/" . $value->user_id, '<i class="fas fa-edit"></i>', ['class' => 'btn btn-sm btn-default btn-text-primary btn-hover-primary']);
  ...
  <tr>
	...
	public function create_klien()
	{
		...
		if ($this->form_validation->run() === TRUE) {
			...
			$induk = $this->db->get_where('users', array('id' => $this->input->post('id_parent_induk')))->row();
			$additional_data = [
				'first_name' => $this->input->post('first_name'),
				'last_name' => $this->input->post('last_name'),
				'company' => $this->input->post('company'),
				'phone' => $this->input->post('phone'),
				'id_parent_induk' => $this->input->post('id_parent_induk'),
				'id_klasifikasi_survei' => $induk->id_klasifikasi_survei,
				'app_id' => $app_id,
				'uuid' => $this->uuid->v4(),
			];
			$additional_data['is_trial'] = '';
		...
		if ($this->form_validation->run() === TRUE && $this->ion_auth->register($identity, $password, $email, $additional_data, $group)) {
			...
			$this->data['id_parent_induk'] = [
				'name' 		=> 'id_parent_induk',
				'id' 		=> 'id_parent_induk',
				'options' 	=> $this->AuthKlien_model->dropdown_parent_induk(),
				'selected' 	=> $this->form_validation->set_value('id_parent_induk'),
				'class' 	=> "form-control",
				'required' => 'required',
			];
	...
	public function edit_klien($id)
	{
		...
		$this->data['id_parent_induk'] = [
			...
			'id' 		=> 'id_parent_induk',
			'options' 	=> $this->AuthKlien_model->dropdown_parent_induk(),
			'selected' 	=> $this->form_validation->set_value('id_parent_induk', $user->id_parent_induk),
			'class' 	=> "form-control",
			'required' => 'required',
		];
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if (!$userInfo) {
				...
				$config = [
					...
					'smtp_port' => 587,
					'smtp_user' => 'survei.kepuasan.online@gmail.com', // change it to yours
					'smtp_pass' => 'rgyteczpthpokkrs', // change it to yours
					'mailtype' => 'html',
					'charset' => 'utf-8',
					// 'wordwrap' => TRUE
				];
				...
				$this->email->from('survei.kepuasan.online@gmail.com', 'Auto Reply SurveiKu.com');
				$this->email->to('abdurrahmanhanif85@gmail.com');
				$this->email->subject('Reset Password');
				$this->email->message($message);


				if ($this->email->send()) {
					$this->session->set_flashdata("success", "Email reset password link sent successfully.");
					redirect("auth/login", 'refresh');
				} else {
