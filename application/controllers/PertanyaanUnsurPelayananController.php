...
class PertanyaanUnsurPelayananController extends CI_Controller
{
	...
	public function ajax_list_unsur_pelayanan()
	{
		...
		foreach ($list as $value) {
			...
			$row[] = anchor('pertanyaan-unsur-pelayanan/edit-unsur/' . $value->id, 'Edit', ['class' => 'btn btn-light-primary btn-sm font-weight-bold shadow']);

			$row[] = '<a class="btn btn-light-primary btn-sm font-weight-bold shadow" href="javascript:void(0)" title="Hapus ' . $value->nama_unsur_pelayanan . '" onclick="delete_data_unsur(' . "'" . $value->id . "'" . ')">Delete</a>';
