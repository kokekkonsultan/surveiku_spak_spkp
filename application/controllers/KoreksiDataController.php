...
class KoreksiDataController extends Client_Controller
{
	...
	public function index($id1, $id2)
	{
		...
		$this->data['jumlah_pertanyaan'] = $this->data['unsur']->num_rows();
	...
	public function ajax_list_data_asli()
	{
		...
		foreach ($list as $value) {
			...
			$row[] = '<b class="text-primary">' . $value->total_skor_spak . '</b>';
			$row[] = '<b class="text-primary">' . $value->total_skor_spkp . '</b>';
	...
	public function ajax_list_data_koreksi()
	{
		...
		foreach ($list as $value) {
			...
			$row[] = '<b class="text-warning">' . $value->total_skor_spak . '</b>';
			$row[] = '<b class="text-warning">' . $value->total_skor_spkp . '</b>';
			// $row[] = $value->nama_lengkap;
	...
	public function _get_data_asli($table_identity)
	{
		...
		$this->data['nilai'] = $this->db->query("SELECT nama_unsur_pelayanan, is_produk,
		...
		(SELECT COUNT(id) FROM unsur_pelayanan_$table_identity up_$table_identity WHERE up_$table_identity.is_produk = unsur_pelayanan_$table_identity.is_produk && id_parent = 0) AS jumlah_pertanyaan_perproduk,
		(((SUM(skor_jawaban)/COUNT(DISTINCT survey_$table_identity.id_responden))/(COUNT(id_parent)/COUNT(DISTINCT survey_$table_identity.id_responden)))/(SELECT COUNT(id) FROM unsur_pelayanan_$table_identity up_$table_identity WHERE up_$table_identity.is_produk = unsur_pelayanan_$table_identity.is_produk && id_parent = 0)) AS rata_rata_bobot
		
		FROM jawaban_pertanyaan_unsur_$table_identity
		JOIN pertanyaan_unsur_pelayanan_$table_identity ON jawaban_pertanyaan_unsur_$table_identity.id_pertanyaan_unsur = pertanyaan_unsur_pelayanan_$table_identity.id
		JOIN unsur_pelayanan_$table_identity ON pertanyaan_unsur_pelayanan_$table_identity.id_unsur_pelayanan = unsur_pelayanan_$table_identity.id
		JOIN survey_$table_identity ON jawaban_pertanyaan_unsur_$table_identity.id_responden = survey_$table_identity.id_responden
		WHERE survey_$table_identity.is_submit = 1 && jawaban_pertanyaan_unsur_$table_identity.skor_jawaban != '0.0'
		GROUP BY id_sub
		ORDER BY is_produk ASC, SUBSTR(nomor_unsur,2) + 0 ASC");
	...
	public function _get_data_koreksi($table_identity)
	{
		...
		AVG(skor_jawaban) AS rata_rata,
		(SELECT nomor_unsur FROM unsur_pelayanan_$table_identity JOIN pertanyaan_unsur_pelayanan_$table_identity ON unsur_pelayanan_$table_identity.id = pertanyaan_unsur_pelayanan_$table_identity.id_unsur_pelayanan WHERE pertanyaan_unsur_pelayanan_$table_identity.id = koreksi_jawaban_pertanyaan_unsur_$table_identity.id_pertanyaan_unsur) AS nomor_unsur,
		(SELECT is_produk FROM unsur_pelayanan_$table_identity JOIN pertanyaan_unsur_pelayanan_$table_identity ON unsur_pelayanan_$table_identity.id = pertanyaan_unsur_pelayanan_$table_identity.id_unsur_pelayanan WHERE pertanyaan_unsur_pelayanan_$table_identity.id = koreksi_jawaban_pertanyaan_unsur_$table_identity.id_pertanyaan_unsur) AS is_produk
		...
		WHERE is_submit = 1 && koreksi_jawaban_pertanyaan_unsur_$table_identity.skor_jawaban != '0.0'
		GROUP BY id_pertanyaan_unsur
		ORDER BY is_produk ASC, SUBSTR(nomor_unsur,2) + 0 ASC");


		$this->data['koreksi_nilai'] = $this->db->query("SELECT nama_unsur_pelayanan, is_produk,
		IF(id_parent = 0, unsur_pelayanan_$table_identity.id, unsur_pelayanan_$table_identity.id_parent) AS id_sub,
		(SUM(skor_jawaban)/COUNT(DISTINCT koreksi_survey_$table_identity.id_responden)) AS rata_rata,
		(COUNT(id_parent)/COUNT(DISTINCT koreksi_survey_$table_identity.id_responden)) AS colspan,
		((SUM(skor_jawaban)/COUNT(DISTINCT koreksi_survey_$table_identity.id_responden))/(COUNT(id_parent)/COUNT(DISTINCT koreksi_survey_$table_identity.id_responden))) AS nilai_per_unsur,
		(SELECT COUNT(id) FROM unsur_pelayanan_$table_identity up_$table_identity WHERE up_$table_identity.is_produk = unsur_pelayanan_$table_identity.is_produk && id_parent = 0) AS jumlah_pertanyaan_perproduk,
		(((SUM(skor_jawaban)/COUNT(DISTINCT koreksi_survey_$table_identity.id_responden))/(COUNT(id_parent)/COUNT(DISTINCT koreksi_survey_$table_identity.id_responden)))/(SELECT COUNT(id) FROM unsur_pelayanan_$table_identity up_$table_identity WHERE up_$table_identity.is_produk = unsur_pelayanan_$table_identity.is_produk && id_parent = 0)) AS rata_rata_bobot
		...
		WHERE koreksi_survey_$table_identity.is_submit = 1 && koreksi_jawaban_pertanyaan_unsur_$table_identity.skor_jawaban != '0.0'
		GROUP BY id_sub
		ORDER BY is_produk ASC, SUBSTR(nomor_unsur,2) + 0 ASC");
	}
